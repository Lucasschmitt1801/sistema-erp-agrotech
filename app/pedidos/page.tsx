...
export default function Pedidos() {
  ...
  const [carrinho, setCarrinho] = useState<any[]>([]) // {id, nome, qtd, preco_unitario, desconto_percentual}
  ...
  // CALCULA TUDO (Total Itens - Desc Itens - Desc Global + Frete)
  ...
  async function salvarOuAtualizar() {
    ...
    const dadosPedido = {
        ...
        valor_total: subtotalItens, // Valor dos produtos (sem frete/desc global)
    ...
    if (pedidoId) {
        ...
        // Remove itens antigos para recriar (jeito mais facil de atualizar lista)
  ...
  async function mudarStatus(novoStatus: string) {
      if(!pedidoId) return alert('Salve o pedido antes de mudar o status.')
      
      if(novoStatus === 'FATURADO') {
          if(!confirm('ATENÇÃO: Isso vai gerar uma VENDA no financeiro, baixar estoque e finalizar o pedido. Confirmar?')) return
          ...
          // 2. Gera Itens da Venda e Baixa Estoque
          for (const item of carrinho) {
               await supabase.from('venda_itens').insert({
                   venda_id: venda.id,
                   produto_id: item.id,
                   quantidade: item.qtd,
                   preco_unitario: item.preco_unitario * (1 - item.desconto_percentual/100) // Preço com desconto
               })
      ...
      {view === 'LISTA' ? (
        ...
        <>
            ...
            <div className="grid gap-4">
                {pedidos.map(p => (
                    <div key={p.id} onClick={() => abrirEditor(p)} className="bg-white p-4 rounded-lg shadow-sm border border-gray-200 flex justify-between items-center cursor-pointer hover:border-blue-400 transition">
                        <div>
            ...
            <div className="lg:col-span-2 space-y-6">
                <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-xl font-bold">
                        {pedidoId ? 'Editar Pedido' : 'Novo Orçamento'} 
                        {pedidoId && <span className={`ml-2 text-sm px-2 py-1 rounded ${statusColors[statusAtual]}`}>{statusAtual}</span>}
                    </h2>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    <select className="w-full border p-3 rounded text-gray-900 bg-white" value={clienteId} onChange={e => setClienteId(e.target.value)}>
                        ...
                        {clientes.map(c => <option key={c.id} value={c.id}>{c.nome_empresa} ({c.contato_nome})</option>)}
                    </select>
                </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                    ...
                    {carrinho.length === 0 ? <p className="text-gray-400 text-center py-4">Nenhum item adicionado.</p> : (
                            ...
                            {carrinho.map(item => (
                                <div key={item.id} className="flex flex-col md:flex-row gap-4 items-center border-b pb-3">
                                    ...
                                    <div className="flex items-center gap-2">
                                        ...
                                        <input type="number" className="w-16 border p-1 rounded text-center" 
                                            value={item.qtd} onChange={e => atualizaItem(item.id, 'qtd', Number(e.target.value))}/>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <label className="text-xs text-gray-500">Desc(%):</label>
                                        <input type="number" className="w-16 border p-1 rounded text-center" 
                                            value={item.desconto_percentual} onChange={e => atualizaItem(item.id, 'desconto_percentual', Number(e.target.value))}/>
                                    </div>
                                    <div className="w-24 text-right font-bold text-gray-700">
                                        R$ {((item.qtd * item.preco_unitario) * (1 - item.desconto_percentual/100)).toFixed(2)}
                                    </div>
                ...
                <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 sticky top-4">
                    ...
                    <div className="space-y-3 text-sm mb-6">
                        ...
                        <div className="flex justify-between items-center">
                            ...
                            <input type="number" className="w-16 border p-1 rounded text-right" 
                                value={valores.descontoGlobal} onChange={e => setValores({...valores, descontoGlobal: Number(e.target.value)})}/>
                        </div>

                        <div className="flex justify-between items-center">
                            <span className="text-gray-500">Frete (R$)</span>
                            <input type="number" className="w-20 border p-1 rounded text-right" 
                                value={valores.frete} onChange={e => setValores({...valores, frete: Number(e.target.value)})}/>
                        </div>
                    ...
                    <div className="mb-4">
                        ...
                        <textarea className="w-full border p-2 rounded h-20 text-sm" 
                            value={valores.observacoes} onChange={e => setValores({...valores, observacoes: e.target.value})}/>
                    </div>

                    {/* BOTÕES DE AÇÃO */}
                    <div className="space-y-3">
                        <button onClick={salvarOuAtualizar} className="w-full bg-blue-600 text-white py-3 rounded-lg font-bold hover:bg-blue-700 flex justify-center gap-2">
                        ...
                        {pedidoId && statusAtual !== 'FATURADO' && (
                            <div className="pt-4 border-t border-gray-100 grid grid-cols-1 gap-2">
                                ...
                                {statusAtual === 'ORCAMENTO' && (
                                    <button onClick={() => mudarStatus('APROVADO')} className="w-full bg-green-100 text-green-700 py-2 rounded font-bold hover:bg-green-200">
                                        Aprovar Orçamento
                                    </button>
                                )}
                                {statusAtual === 'APROVADO' && (
                                    <button onClick={() => mudarStatus('PRODUCAO')} className="w-full bg-purple-100 text-purple-700 py-2 rounded font-bold hover:bg-purple-200">
                                        Enviar para Produção
                                    </button>
                                )}
                                {statusAtual === 'PRODUCAO' && (
                                    <button onClick={() => mudarStatus('ENVIADO')} className="w-full bg-orange-100 text-orange-700 py-2 rounded font-bold hover:bg-orange-200">
                                        Marcar como Enviado
                                    </button>
                                )}
                                {(statusAtual === 'ENVIADO' || statusAtual === 'APROVADO') && (
                                    <button onClick={() => mudarStatus('FATURADO')} className="w-full bg-gray-800 text-white py-3 rounded font-bold hover:bg-black flex justify-center gap-2">
                                        <CheckCircle size={18}/> Faturar (Gerar Venda)
                                    </button>
                                )}
                                <button onClick={() => mudarStatus('CANCELADO')} className="w-full text-red-500 text-xs hover:underline mt-2">
                                    Cancelar Pedido
                                </button>
                            </div>
                        )}
                        ...
                        {statusAtual === 'FATURADO' && (
                            <div className="bg-green-50 text-green-700 p-3 rounded text-center text-sm font-bold border border-green-200">
                                Pedido Faturado e Encerrado ✅
                            </div>
                        )}
                    </div>
                </div>
